...
public:
    ...
    bool insert(int val) {
        ...
        m_rnd.emplace_back(val);
        this->m_vals.emplace(std::make_pair(val, m_rnd.size()-1));
        return true;
    }
    ...
    bool remove(int val) {
        ...
        int temp = m_rnd[m_rnd.size()-1];
        m_rnd[m_rnd.size()-1] = m_rnd[this->m_vals[val]];
        m_rnd[this->m_vals[val]] = temp;
        this->m_vals[temp] = this->m_vals[m_rnd[m_rnd.size()-1]];
    ...
    int getRandom() {
        int random = rand()%this->m_rnd.size();
        return m_rnd[random];
    }
private:
    std::unordered_map<int, int> m_vals;
    std::vector<int> m_rnd;
};
